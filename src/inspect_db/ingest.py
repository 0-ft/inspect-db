...
class IngestionProgressListener[FileId](Protocol):
    ...
    def on_file_finished(self, file_id: FileId, status: Literal["success", "skipped", "error"], message: str) -> None:
...
class NullProgressListener(IngestionProgressListener):
    ...
    def on_file_finished(self, file_id: Any, status: Literal["success", "skipped", "error"], message: str) -> None:
        pass
...
def get_db_stats(database_uri: str) -> Dict[str, Any]:
    ...
    with db.session() as session:
        ...
        # Count samples
        sample_count = session.exec(select(func.count()).select_from(DBEvalSample)).one()
        
        # Count messages
        message_count = session.exec(select(func.count()).select_from(DBChatMessage)).one()
        
        # Get average samples per log
        avg_samples = session.exec(
            select(func.avg(
                select(func.count())
                .select_from(DBEvalSample)
                .where(DBEvalSample.log_uuid == DBEvalLog.uuid)
                .scalar_subquery()
            ))
        ).one() or 0
        
        # Get average messages per sample
        avg_messages = session.exec(
            select(func.avg(
                select(func.count())
                .select_from(DBChatMessage)
                .where(DBChatMessage.sample_uuid == DBEvalSample.uuid)
                .scalar_subquery()
            ))
        ).one() or 0
        ...
        role_counts = session.exec(
            select(DBChatMessage.role, func.count())
            .group_by(DBChatMessage.role)
        ).all()
...
def process_eval_file(eval_file: Path, db: EvalDB, progress: IngestionProgressListener) -> bool:
    ...
    try:
        ...
        progress.on_file_finished(file_id, "success", f"Ingested {eval_file.name} ({log_uuid})")
        return True
    except IntegrityError:
        progress.on_file_finished(file_id, "skipped", f"Skipped {eval_file.name} (already exists)")
        return False
...
def ingest_eval_files(
    ...
    for path in eval_paths:
        eval_files.extend([Path(f) for f in glob(str(path), recursive=True) if f.endswith('.eval')])
    ...
    with progress_listener.progress():
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = [executor.submit(worker, q, db, progress_listener) for _ in range(workers)]
