...
class RichProgressListener(IngestionProgressListener[TaskID]):
    ...
    def __init__(self):
        ...
        self.task_ids = {}
        self.TaskID = TaskID  # Store TaskID type for type hints
    ...
    def on_file_started(self, file_path: Path) -> TaskID:
        """Start tracking progress for a file."""
        task_id = self.rich_progress.add_task(f"Processing {file_path.name}", total=1)
        self.task_ids[file_path] = task_id
        self.started_count += 1
        return task_id
    
    def on_file_finished(self, file_id: TaskID, status: Literal["success", "skipped", "error"], message: str) -> None:
        """Update progress for a completed file."""
        self.rich_progress.update(file_id, advance=1, description=message)
        if status == "success":
...
def ingest(database_uri: str, eval_paths: list[str | Path], workers: int, quiet: bool) -> None:
