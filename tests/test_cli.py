...
def test_ingest_command(
    db_uri: str, sample_eval_log_path: Path, mock_progress_listener, mocker
):
    """Test the ingest command."""
    runner = CliRunner()

    mocker.patch(
        "inspect_db.cli.RichProgressListener", return_value=mock_progress_listener
    )
    ...
    mock_progress_listener.on_log_completed.assert_called_once_with(
        sample_eval_log_path, "success", mocker.ANY
    )
...
def test_ingest_command_multiple_files(
    db_uri: str,
    temp_dir: Path,
    sample_eval_log: EvalLog,
    ...
    # Verify progress listener was used correctly
    assert mock_progress_listener.on_log_started.call_count == 3
    assert mock_progress_listener.on_log_completed.call_count == 3
    mock_progress_listener.on_ingestion_complete.assert_called_once()
...
def test_ingest_command_duplicate_files(
    db_uri: str, sample_eval_log_path: Path, mock_progress_listener, mocker
):
    ...
    mock_progress_listener.on_log_started.assert_called_with(sample_eval_log_path)
    mock_progress_listener.on_log_completed.assert_called_with(
        sample_eval_log_path, "skipped", mocker.ANY
    )
...
def test_ingest_command_invalid_file(
    db_uri: str, temp_dir: Path, mock_progress_listener, mocker
):
    ...
    # Verify progress listener was used correctly
    mock_progress_listener.on_log_started.assert_called_once_with(invalid_file)
    mock_progress_listener.on_log_completed.assert_called_once_with(
        invalid_file, "error", mocker.ANY
    )
    mock_progress_listener.on_ingestion_complete.assert_called_once()


def test_ingest_command_with_workers(
    db_uri: str,
    temp_dir: Path,
    sample_eval_log: EvalLog,
    mock_progress_listener,
    ...
    result = runner.invoke(
        cli, ["ingest", "--workers", "2", db_uri, str(temp_dir / "*.eval")]
    )

    assert result.exit_code == 0

    # Verify progress listener was used correctly
    assert mock_progress_listener.on_log_started.call_count == 5
    assert mock_progress_listener.on_log_completed.call_count == 5
    mock_progress_listener.on_ingestion_complete.assert_called_once()
...
def test_ingest_command_quiet(
    ...
    result = runner.invoke(
        cli, ["ingest", "--quiet", db_uri, str(sample_eval_log_path)]
    )
